...
2. Run `cd logzio_aws_serverless/python2/cloudwatch/`. Zip 'lambda_function.py' and 'shipper.py' using the following command(s):
    - `mkdir -p dist/python2/shipper; cp -r ../shipper/* dist/python2/shipper && cp src/lambda_function.py dist && cp src/__init__.py dist/python2 && cd dist/ && zip logzio-cloudwatch lambda_function.py python2/__init__.py python2/shipper/*`
...
5. In the **Environment variables** section, set your Logz.io account token, URL, and log type:
    ...
    | `TYPE` | The log type you'll use with this Lambda. Please note that you should create a new Lambda for each log type you use. This can be a [built-in log type](https://docs.logz.io/user-guide/log-shipping/built-in-log-types.html), or your custom log type | `hyperdx_cloudwatch_lambda`|
...
4. The following is an example on how to deploy CloudFormation. Replace the parameters with the ones you need:
    ...
    | `LogzioTYPE` | The log type you'll use with this Lambda. Please note that you should create a new Lambda for each log type you use. This can be a [built-in log type](https://docs.logz.io/user-guide/log-shipping/built-in-log-types.html), or your custom log type | `hyperdx_cloudwatch_lambda` |
